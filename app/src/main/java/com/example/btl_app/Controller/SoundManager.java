...
public class SoundManager {
    ...
    public static void playWrongSound(Context context) {
        SharedPreferences prefs = context.getSharedPreferences("GameSettings", Context.MODE_PRIVATE);
        if (prefs.getBoolean("soundEnabled", true) && prefs.getBoolean("soundWrong", true)) {
            if (bgMediaPlayer != null && bgMediaPlayer.isPlaying()) {
                bgMediaPlayer.pause();
            }

            MediaPlayer wrongPlayer = MediaPlayer.create(context, R.raw.wrong_sound);
            wrongPlayer.start();
            wrongPlayer.setOnCompletionListener(MediaPlayer::release);
        }
    }
